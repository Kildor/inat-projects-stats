...
export default class extends Module {
	constructor() {
		...
		"order_by", "strategy", "difference"], this.state,{}, {
			order_by: {setting: 'order_by', save: true, type: 'select', default: 0},
	...
	async counter() {
		...
		if (!!limit) customParams['limit'] = limit;
		if (!!additional) {
		...
		const newObservers = await API.fetchObservers(customNewParams, limit, this.setStatusMessage, customParams);

		console.dir(newObservers);
		...
		const prevObservers = await API.fetchObservers(customOldParams, 0, this.setStatusMessage, customParams);

		console.dir(prevObservers);
